...
void main() {
  ...
  test('attempt success and later failure preserve last successful metadata', () async {
    ...
    final row = await (database.select(database.syncStates)
          ..where((table) => table.resourceKey.equals('station|id=station-1')))
        .getSingle();
  ...
  test('new success clears previous failure without erasing optional metadata', () async {
