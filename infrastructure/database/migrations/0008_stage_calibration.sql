...
BEGIN
  IF OLD.status <> 'CANDIDATE' AND (
    ...
    RAISE EXCEPTION 'activated/superseded rating-curve scientific fields are immutable';
  END IF;
  RETURN NEW;
END;
$$;
...
BEGIN
  target_curve_id := COALESCE(NEW.rating_curve_id, OLD.rating_curve_id);
  ...
  IF curve_status <> 'CANDIDATE' THEN
    RAISE EXCEPTION 'rating-curve points are immutable after activation/supersession';
  END IF;
